...
3.Just-In-Time コンパイラ
- 実行前にJavaコンパイルされた、中間言語を実行時に機械語にコンパイルする
- メソッド単位でコンパイルする
...
1. データと部分的な結果を保存するために使用されます。
   1. メソッドと、プリミティブ型のデータが保存される
   2. メソッドの値を返したり動的リンクを実行したりする際に必要となるデータと部分的な結果を保存するために使用されます。
2. スタック用のメモリは連続している必要はありません。

動作：https://speakerdeck.com/deepu105/jvm-memory-usage-stack-vs-heap
