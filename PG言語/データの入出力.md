- [データ入出力の基礎知識](#データ入出力の基礎知識)
  ...
  - [クラス構成](#クラス構成)
    - [入出力の対](#入出力の対)
    - [読み込むデータ種類](#読み込むデータ種類)
- [実際のクラス（バイトデータを扱うクラス）](#実際のクラスバイトデータを扱うクラス)
...
- [実際のクラス（文字データを扱うクラス）](#実際のクラス文字データを扱うクラス)
  - [Reader](#reader)
    - [StringReader](#stringreader)
    - [CharArrayReader](#chararrayreader)
    - [FileReader](#filereader)
    - [PipedReader](#pipedreader)
  - [文字入力用のラッパストリーム（InputStream のラッパストリーム）](#文字入力用のラッパストリームinputstream-のラッパストリーム)
    - [InputStreamReader](#inputstreamreader)
  - [文字入力用のラッパストリーム（Reader のラッパストリーム）](#文字入力用のラッパストリームreader-のラッパストリーム)
    - [FilterReader](#filterreader)
    - [BuffaredReader](#buffaredreader)
    - [LineNumberReader](#linenumberreader)
  - [Writer](#writer)
    - [FileWriter](#filewriter)
  - [文字出力用のラッパストリーム（バイトデータ出力ストリームをラップするもの）](#文字出力用のラッパストリームバイトデータ出力ストリームをラップするもの)
    - [OutputStreamWriter](#outputstreamwriter)
  - [文字出力用のラッパストリーム（Writerをラップするもの）](#文字出力用のラッパストリームwriterをラップするもの)
...
2. バイナリデータ(バイトデータ)：画像、動画、音声、数値データのようなバイト単位のデータ。
...
### FilterReader
Readerに機能を追加するラッパクラス。

### BuffaredReader
Readerにバッファ機能を追加したラッパクラス。

read()
  - テキストファイルから1行分のテキストをまとめて読み込みます。
  - 改行文字は読み込んだ文字に含まれない事です。
...
### LineNumberReader
BufferedReaderのサブクラスで，行番号を認識するラッパリー
ダである。


## Writer
すべての文字データ出力ストリームのスーパークラスとして，抽象クラス Writer が用意され
ている。このため，文字データ出力ストリームのことを，単に“ライタ”と呼ぶこともある。

### FileWriter
ファイルを出力先とするライタ。

## 文字出力用のラッパストリーム（バイトデータ出力ストリームをラップするもの）
### OutputStreamWriter
OutputStreamWriter は， InputStreamReader の出力版で，OutputStream をラップして，
Unicode文字データから，別のエンコーディングへ変換したデータを書き込む機能を持ってい
るライタである(Fig. 3-2参照)。

## 文字出力用のラッパストリーム（Writerをラップするもの）
### FilterWriter
### BufferedWriter
### PrintWriter
